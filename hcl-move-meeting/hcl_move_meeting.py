#!/usr/bin/env python3
"""
掃收件匣找出同仁回覆接受的會議通知信件（含討論串），逐一移到「會議通知」資料匣。
"""
import json, os
...
def _scan_visible_matches(page, seen):
    """掃當前 DOM，回傳新發現的 [{sender, subject, keyword}, ...]，已見過的跳過。"""
    new_items = []
    for item in page.locator('[role="treeitem"]').all():
        ...
        new_items.append({"sender": sender, "subject": subject, "keyword": kw_hit})
    return new_items
...
def move_to_folder(page, item, sender, subject):
    ...
    folder_input.click()
    # v1.2.2：先 fill 強制清空 + 設值（避免 IME 殘留），再 type 觸發 React onChange
    folder_input.fill("")
    folder_input.type(TARGET_FOLDER, delay=50)
    page.wait_for_timeout(1000)

    # v1.2.2：明確比對資料夾名稱（含 TARGET_FOLDER 字串）— 舊版用 .first 可能選錯
    # 例如過濾沒生效時 .first 會選到「Inbox」或第一個無關項目，導致回報 moved 但實際沒移動
    folder_item = page.locator(
        f"div.folder-tray-float.show [role='treeitem']:visible:has-text('{TARGET_FOLDER}')"
    ).first
    try:
        ...
        # fallback：找不到含資料夾名稱的項目 → 按 Enter 讓 input 自己確認
        print(f"  ⚠️ 找不到含「{TARGET_FOLDER}」的項目，改按 Enter")
        folder_input.press("Enter")
        page.wait_for_timeout(1500)

    # v1.2.2：驗證 popup 是否關閉（成功移動會關閉 popup）
    if page.locator("div.folder-tray-float.show").count() > 0:
        print(f"  ⚠️ 資料夾 popup 未關閉，移動可能失敗：{sender} / {subject[:40]}")
        # 嘗試補按 Enter
        try:
            folder_input.press("Enter")
            page.wait_for_timeout(1500)
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for i, mail in enumerate(mails, 1):
            ...
            status = move_to_folder(page, item_el, sender, subject)
    ...
    with open(os.path.join(tempfile.gettempdir(), "hcl_move_meeting.json"), "w") as f:
