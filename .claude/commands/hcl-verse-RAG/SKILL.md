...
6. 每則訊息各自：
   ...
   - **分支 A — ② Hindsight retain**：`retain` 到 `EID` bank（明確帶 `bank_id="EID"`），
     `document_id`=UNID，**不帶 `tags`**（已移除，retain() 呼叫不再傳這個參數），
...
歸檔階段先不分類 `proj:`。`retain()` 目前**不帶 `tags` 參數**（已移除），之後要補分類時：
`document_id` 是可重算的固定值（UNID）、Hindsight `retain` 是 upsert（同 id 直接覆蓋）、
`subject`/`body` 已經留在 Qdrant payload 裡 → 寫一支 backfill 腳本可以直接從 Qdrant 撈，
不用重新爬 Verse，兩件事完全解耦。因為 `tags` 已經整個拿掉，屆時要嘛在 `retain()` 重新
加回 `tags` 參數傳 `proj:xxx`，要嘛改用 `metadata` 存分類——兩種都可行，實際做的時候再定。
`project_keywords.py` 的 `match_projects()`（多標籤加權）/`match_project()`
...
- **不帶 `tags`**（已移除，`retain()` 呼叫不再傳這個參數；proj 分類暫緩，見上方章節）
