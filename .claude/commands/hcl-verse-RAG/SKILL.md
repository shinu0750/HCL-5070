...
- **🚨 最優先待處理（2026-07-13）：ycmu（彥池）帳號的 Verse 已經關掉討論串分組**，
  這支 pipeline 目前完全沒有針對這個新畫面驗證過，繼續在這個帳號上跑歸檔前一定要
  先確認：
  - **怎麼關的**：Verse 工具列有個按鈕 `[class*='toggle-threads']`（title="Don't
    group email conversations as a thread"），已經在「工程專案 > JSR量產建置」
    資料夾點下去並確認生效——點擊前資料夾清單每一列可能是多則訊息合併的討論串
    （帶 `Count\nN`），點擊後**每一列變成單一則訊息**，不再合併、不再有 `Count`
    （例如原本合併成一列的「Python安裝」討論串，現在拆成「回覆: Python安裝」
    好幾列各自獨立）
  - **這是帳號層級設定**，不是只影響這個資料夾，理論上 ycmu 帳號所有資料夾都會
    受影響（含以後新增的資料夾），但只在 JSR量產建置 資料夾實際驗證過，其他資料夾
    沒有逐一確認
  - **為什麼這件事很重要**：現在整支 `verse_archive_pipeline.py` 的訊息級抓取邏輯
    ...
    一則的內容），沒驗證過不能假設**
  - **下一步**：在正式對 ycmu 帳號跑任何批次歸檔之前，先用 `--no-move` 對
    「工程專案 > JSR量產建置」（或任何資料夾）跑一次，仔細核對印出來的
