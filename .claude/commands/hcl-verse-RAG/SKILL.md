...
6. 每則訊息各自：
   - **分支 A — ① RAG**：本地 `jina-embed`（llama-cpp-server，OpenAI-compatible API，
     ...
     （`[{name, path}, ...]`，`path` 指向另存在 `ATTACHMENTS_DIR`（`~/verse-export/attachments/`）
     的實體檔案，檔名前綴 unid 避免同名衝突——跟內嵌在 `.eml` 裡的附件是同一份資料
     另存一份，不是重新下載）
   - **分支 A — ② Hindsight retain**：`retain` 到 `EID` bank，`document_id`=UNID，
...
- `metadata` = `{subject, from_email, from_name, to, cc, thread_id, unid, reply_to_unid, sent_date}`
  ...
  指標還是能把整串前後文串回去
- `bank_id` = `EID`（`verse_archive_pipeline.py` 沒有明講 bank_id，用的是 Hindsight
  server 端的預設值，實測就是 `EID`；`gmail_backfill.py` 原本寫死 `shuhsing` 是舊機器
  遺留的錯誤預設，已改成 `EID` 保持一致）
- Qdrant payload 額外多一個 `attachments` 欄位（`[{name, path}, ...]`），`path` 指向
  `~/verse-export/attachments/` 裡另存的實體檔案（檔名前綴 unid）
