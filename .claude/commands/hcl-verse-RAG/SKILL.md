...
- **討論串分組開關不可信任「之前關過就會一直關著」，每次都要重新檢查**（2026-07-13
  當天稍晚更新）：先前這裡記錄「三個帳號都已關閉討論串分組，ycmu 的
  「工程專案 > JSR量產建置」已驗證生效」——但同一天稍晚用 headful 模式實際打開
  這個資料夾檢查，畫面上清清楚楚看到每一列都有數字徽章（`Count N`，實測
  7/4/5/47/7/20/5），代表分組**其實是開著的**，跟先前記錄矛盾。點擊
  `[class*='toggle-threads']` 後徽章消失、該資料夾郵件數從分組列數變成 296
  則個別訊息，確認切換本身仍然有效，只是**開關狀態不能只憑之前記錄的結論假設，
  一定要當次「先查、再決定要不要點」**（下面的正確流程其實一直都這樣寫，這次只是
  印證了「先查」這一步不能省略，即使記錄說已經處理過也一樣）。原因不明——可能是
  帳號設定其實不是真的持久生效、被其他登入/清 cache 重置、或是先前的驗證記錄一開始
  就有誤，沒有進一步查證
  - **同時這次意外驗證到一個原本標成「未驗證」的疑慮**：先前擔心「分組關閉後，一列
    變成一則訊息本身，沒有手風琴可展開，訊息級抓取邏輯可能整個失效」——但這次同一個
    資料夾在切換前（分組開）用 `--no-move` 抓到「Python安裝」正確判定 7 則訊息
    （對應畫面上的 `7` 徽章），跟切換後另外兩封信（1 則、3 則）也都正確對應，代表
    `open_row_and_get_block_unids()`/`extract_message_block()`（讀 preview pane 的
    accordion 手風琴數量，不是讀列表的 `Count` 徽章）在**分組開/關兩種畫面下都正確**
    ——這個抓取邏輯本來就不依賴列表層級的分組顯示，原本的疑慮可以打消，不用再當作
    未知風險看待
  - 以下段落是原本記錄的驗證內容，供參考（**但「已經關閉」這個結論本身已被上面的
    發現推翻，不要直接採信，每次用前務必自己檢查一次**）：
    **ycmu 帳號的「工程專案 > JSR量產建置」資料夾已於 2026-07-13 用 `--no-move`
  +正式歸檔 2 封信端對端驗證過**：關掉分組後的畫面下，訊息級抓取邏輯（`Count\nN`
  討論串正確拆成對應則數，實測 7 則訊息的討論串正確抓出 7 則，RAG/Hindsight 都
  7/7）運作正常，不是理論上的假設。**其他帳號/資料夾組合仍未逐一驗證**，第一次
  對新帳號/新資料夾正式跑批次歸檔前，建議還是先用 `--no-move` 確認
  `message_count`/`RAG`/`Hindsight` 數字合理：
  - **怎麼關的**：Verse 工具列有個按鈕 `[class*='toggle-threads']`（title="Don't
    group email conversations as a thread"）。正確流程是**先檢查再決定要不要
    點**，不是無腦直接點（已關閉的狀態下再點一次會變回重新開啟討論串分組）：
    先看資料夾清單前幾列有沒有 `Count\nN`（有 = 目前是分組狀態），有才點擊這個
    按鈕，點完再重新檢查一次確認 `Count` 真的消失了。三個帳號（shuhsing/tzuyu 的
    Inbox、ycmu 的「工程專案 > JSR量產建置」資料夾）都照這個「先查、再點、再驗證」
    的順序做過，點擊前都確認過當下確實是分組狀態
  - **這是帳號層級設定**，會影響該帳號所有資料夾（含以後新增的資料夾），三個帳號
    都只在單一資料夾/Inbox 檢查+點擊，沒有逐一確認該帳號底下每個資料夾都生效
    （合理假設整個帳號都套用了，但沒有窮舉驗證）。**之後如果有新帳號要用這支
    pipeline，一樣要照上面「先檢查、有分組才點」的順序處理**，不要假設新帳號
    預設值跟現有三個帳號一樣，也不要不查就直接點
