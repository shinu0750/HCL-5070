...
def _scan_visible_matches(page, seen):
    """???DOM嚗??單?潛??[{sender, subject, keyword}, ...]嚗歇閬??歲??""
    new_items = []
...
def move_to_folder(page, item, sender, subject):
    ...
    folder_input.click()
    # v1.2.2嚗? fill 撘瑕皜征 + 閮剖潘??踹? IME 畾?嚗???type 閫貊 React onChange
    folder_input.fill("")
    folder_input.type(TARGET_FOLDER, delay=50)
    page.wait_for_timeout(1000)

    # v1.2.2嚗?蝣箸?撠??冗?迂嚗 TARGET_FOLDER 摮葡嚗?????.first ?航?賊
    # 靘??蕪瘝??? .first ??啜nbox??蝚砌?????殷?撠? moved 雿祕??蝘餃?
    ...
    try:
        ...
        # fallback嚗銝?怨??冗?迂????????Enter 霈?input ?芸楛蝣箄?
        print(f"  ?? ?曆??啣?TARGET_FOLDER}???嚗??Enter")
        folder_input.press("Enter")
        page.wait_for_timeout(1500)

    # v1.2.2嚗?霅?popup ?臬??嚗??宏???? popup嚗?
    if page.locator("div.folder-tray-float.show").count() > 0:
        print(f"  ?? 鞈?憭?popup ?芷???蝘餃??航憭望?嚗sender} / {subject[:40]}")
        # ?岫鋆? Enter
        try:
            folder_input.press("Enter")
            page.wait_for_timeout(1500)
